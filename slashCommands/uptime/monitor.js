  ...
  run: async (client, interaction, args) => {
    ...
    if (checkIfExsists === null) {
      ...
      await UrlsConfig.create({
        ...
        await interactionA.edit({ embeds: [new MessageEmbed().setTitle("<a:tickwhite:885856951072874566> Added Succesfully").setColor("RED").setDescription("Thanks for using me").setTimestamp()] });
        
      });
    } else {      
      
      await interactionA.edit({ embeds: [new MessageEmbed().setTitle("❌ | Already Registered").setDescription("The Project you're Trying To Register Is Already In The Database").setColor("RED").setFooter(interaction.member.user.tag).setThumbnail(client.user.displayAvatarURL()).setTimestamp()] });
    }
  },
};
